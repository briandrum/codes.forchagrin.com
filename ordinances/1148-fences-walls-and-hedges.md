...
1. {% comment -%} (a) {%- endcomment -%} To regulate the location, height and overall general characteristics
...
2. {% comment -%} (b) {%- endcomment -%} To maintain an orderly appearance and reduce any negative impact upon
other properties where such fencing, walls, or hedges are visible.
3. {% comment -%} (c) {%- endcomment -%} To prohibit fence, wall or hedge placement at heights or locations
which interfere with clear visibility of vehicular and pedestrian traffic on
adjacent streets, alleys, or sidewalks.
4. {% comment -%} (d) {%- endcomment -%} To preserve and protect public health, safety and welfare by
...
1. {% comment -%} (a) {%- endcomment -%} "Fence" means a barrier open to light and air which is constructed of
non-living materials in order to enclose, screen, or secure property, or, a
means of designating a property boundary.
2. {% comment -%} (b) {%- endcomment -%} "Hedge" means an evergreen or deciduous landscape barrier consisting
of a continuous, dense planting of shrubs which have several small stems from
the ground or small branches near the ground.
3. {% comment -%} (c) {%- endcomment -%} "Retaining wall" means a solid barrier which is constructed of stone
or masonry which functions to resist a lateral load from a mass of earth or
other material.
4. {% comment -%} (d) {%- endcomment -%} "Wall" means a solid barrier which is constructed of wood, masonry or
...
1. {% comment -%} (a) {%- endcomment -%} Construction. Fences and walls must be of uniform design and
constructed of durable materials, maintained in good condition, and not
permitted to become dilapidated.
2. {% comment -%} (b) {%- endcomment -%} Appearance. The unfinished side, supporting rails, and posts of any
...
3. {% comment -%} (c) {%- endcomment -%} Location.
    1. {% comment -%} (1) {%- endcomment -%} Any fence designed with a surface at least twenty-five percent
(25%) open to light and air throughout its area may be located on a property
line.
    2. {% comment -%} (2) {%- endcomment -%} Any wall, any hedge, or, a fence designed with a surface of less
...
be located no less than three feet from a property line.
    3. {% comment -%} (3) {%- endcomment -%} (Editor’s note: Former subsection (c)(3) was repealed by Ordinance
2020-10, passed March 9, 2020.)
4. {% comment -%} (d) {%- endcomment -%} Prohibited fences, walls and hedges.
    1. {% comment -%} (1) {%- endcomment -%} Fences charged with electrical current.
    2. {% comment -%} (2) {%- endcomment -%} Barbed wire fences except as otherwise provided for in Section 1148.06(b).
    3. {% comment -%} (3) {%- endcomment -%} Retaining walls constructed of plain concrete masonry unit blocks
without aesthetic additive such as exposed aggregate or thoroughly blended
coloration.
5. {% comment -%} (e) {%- endcomment -%} Proximity to Historic Structures. Fences located in proximity to
historically significant properties shall be Compatible in design with the
character of such properties.
6. {% comment -%} (f) {%- endcomment -%} Utility Uses. Utility uses shall be completely enclosed by a fence or
...
7. {% comment -%} (g) {%- endcomment -%} Exceptions. The requirements of this chapter shall not apply to a
...
1. {% comment -%} (a) {%- endcomment -%} Permit Required. No person shall erect any fence,  wall, or retaining
...
2. {% comment -%} (b) {%- endcomment -%} Survey Required.  If documentation of the property line locations is
not conclusive, the Administrator may require a professional survey to identify
such locations prior to issuing a permit.  (Ord. 2019-39.  Passed 8-13-19.)

## 1148.05   Fences, walls, and hedges permitted:  residential districts

<p class="Markdown-list--a-1-A"></p>

1. {% comment -%} (a) {%- endcomment -%} Front Yards.  Fences, walls and hedges shall not exceed three and one-
...
2. {% comment -%} (b) {%- endcomment -%} Side and Rear Yards.  Fences, walls and hedges shall not exceed six
...
3. {% comment -%} (c) {%- endcomment -%} Corner Lots.  The installation of a fence, wall or combination thereof
...
4. {% comment -%} (d) {%- endcomment -%} Recreational Uses.
    1. {% comment -%} (1) {%- endcomment -%} Swimming pools.  Any swimming pool in which water may collect to a
...
actual use or is unattended.
    2. {% comment -%} (2) {%- endcomment -%} Tennis courts.  Tennis courts shall be enclosed by a metal chain
...
1. {% comment -%} (a) {%- endcomment -%} Height.  Fences, walls, and hedges shall not exceed eight feet in
...
2. {% comment -%} (b) {%- endcomment -%} Limited Industrial District.  In the Limited Industrial District, the
...
3. {% comment -%} (c) {%- endcomment -%} Contiguous Residential Districts.  The Planning and Zoning Commission
...
1. {% comment -%} (a) {%- endcomment -%} A retaining wall may be constructed on property where topographic
conditions warrant or where necessary to retain fill or cut slopes.
2. {% comment -%} (b) {%- endcomment -%} In addition to the permit requirements of Section 1148.04, all retaining walls over three (3) feet in height shall require the
...
3. {% comment -%} (c) {%- endcomment -%} Retaining walls shall be set back from any property line a minimum of
three (3) feet plus one (1) foot for every foot of wall height greater than
three (3) feet.
4. {% comment -%} (d) {%- endcomment -%} The height of a retaining wall shall be measured vertically from the
finished grade of the lower side of the wall to the highest point at the top of
the wall.
5. {% comment -%} (e) {%- endcomment -%} Retaining walls exceeding three (3) feet in height shall have a hedge,
fence or rail at the top edge of the wall.
6. {% comment -%} (f) {%- endcomment -%} Retaining walls shall not restrict access to utilities, impede the
normal flow of storm water, cross open drainage channels, or be located over an
access easement.
7. {% comment -%} (g) {%- endcomment -%} Retaining walls shall be maintained by the owner in good repair at all
times.
8. {% comment -%} (h) {%- endcomment -%} Walls greater than six (6) feet in height or in excess of one-hundred
