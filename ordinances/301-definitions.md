...
agricultural purposes.  (ORC 4511.01(J))
...
declared an "alley" by Council.  (ORC 4511.01(XX))

### 301.031   Beacon; hybrid beacon

<p class="Markdown-list--a-1-A"></p>

1. {% comment -%} (a) {%- endcomment -%} “Beacon” means a highway traffic signal with one or more signal
sections that operate in a flashing mode.  
(ORC 4511.01(KKK))
2. {% comment -%} (b) {%- endcomment -%} “Hybrid beacon” means a type of beacon that is intentionally placed in
...
1. {% comment -%} (a) {%- endcomment -%} "Bicycle" means every device, other than a device that is designed
...
(ORC 4511.01(G))
2. {% comment -%} (b) {%- endcomment -%} "Motorized bicycle" or “moped” means any vehicle having either two
...
4511.01(H))
3. {% comment -%} (c) {%- endcomment -%} “Electric bicycle” means a “class 1 electric bicycle”, a “class 2
electric bicycle”, or a “class 3 electric bicycle” as defined in this section.  
(ORC 4511.01(RRR))
    1. {% comment -%} (1) {%- endcomment -%} “Class 1 electric bicycle” means a bicycle that is equipped with
...
hour.  (ORC 4511.01(SSS))
    2. {% comment -%} (2) {%- endcomment -%} “Class 2 electric bicycle” means a bicycle that is equipped with
fully operable pedals and an electric motor of less than seven hundred fifty
...
of twenty miles per hour.  (ORC 4511.01(TTT))
    3. {% comment -%} (3) {%- endcomment -%} “Class 3 electric bicycle” means a bicycle that is equipped with
fully operable pedals and an electric motor of less than seven hundred fifty
watts that provides assistance only when the rider is pedaling and ceases to
...
portion of such other vehicles, or the load thereon, or both.  (ORC 4511.01(I))
...
such street or highway.  (ORC 4511.01(CC))
...
1. {% comment -%} (a) {%- endcomment -%} That part of a roadway at intersections ordinarily included within the
...
2. {% comment -%} (b) {%- endcomment -%} Any portion of a roadway at an intersection or elsewhere, distinctly
indicated for pedestrian crossing by lines or other markings on the surface;
3. {% comment -%} (c) {%- endcomment -%} Notwithstanding subsections (a) and (b) hereof, there shall not be a
...
designed for use in specific highway maintenance activities.  (ORC 4511.01
(QQQ))
...
1. {% comment -%} (a) {%- endcomment -%} The area embraced within the prolongation or connection of the lateral
...
2. {% comment -%} (b) {%- endcomment -%} If a highway includes two roadways that are thirty feet or more apart,
...
3. {% comment -%} (c) {%- endcomment -%} At a location controlled by a traffic control signal, regardless of
the distance between the separate intersections as described in subsection (b)
of this section:
    1. {% comment -%} (1) {%- endcomment -%} If a stop line, yield line, or crosswalk has not been designated on
...
intersections and the roadway and median constitute one intersection.
    2. {% comment -%} (2) {%- endcomment -%} Where a stop line, yield line, or crosswalk line is designated on
...
intersection.
    3. {% comment -%} (3) {%- endcomment -%} Where a crosswalk is designated on a roadway on the departure from
...
interchanges, and at opposite approaches of the same intersection.  (ORC
4511.01(NNN))
...
1. {% comment -%} (a) {%- endcomment -%} A violation of Ohio R.C. 4511.03, 4511.051, 4511.12, 4511.132,
...
4511.78 or 4511.84;
2. {% comment -%} (b) {%- endcomment -%} A violation of division (A)(2) of Ohio R.C. 4511.17, divisions (A) to
(D) of Ohio R.C. 4511.51, or division (A) of Ohio R.C. 4511.74;
3. {% comment -%} (c) {%- endcomment -%} A violation of any provision of Ohio R.C. 4511.01 to 4511.76 for which
no penalty otherwise is provided in the section that contains the provision
violated;
4. {% comment -%} (d) {%- endcomment -%} A violation of Ohio R.C. 4511.214.
5. {% comment -%} (e) {%- endcomment -%} A violation of a municipal ordinance that is substantially similar to
...
1. {% comment -%} (a) {%- endcomment -%} "Private road or driveway" means every way or place in private
...
2. {% comment -%} (b) {%- endcomment -%} “Private road open to public travel” means a private toll road or
...
aisle within a parking area, or a private grade crossing.  (ORC 4511.01(OOO))
...
1. {% comment -%} (a) {%- endcomment -%} Ambulances, including private ambulance companies under contract to a
...
2. {% comment -%} (b) {%- endcomment -%} Motor vehicles used by public law enforcement officers or other
persons sworn to enforce the criminal and traffic laws of the State or the
Municipality;
3. {% comment -%} (c) {%- endcomment -%} Any motor vehicle when properly identified as required by the Ohio
...
4. {% comment -%} (d) {%- endcomment -%} Vehicles used by fire departments, including motor vehicles when used
...
(ORC 4511.01(E))
5. {% comment -%} (e) {%- endcomment -%} Vehicles used by the Commercial Motor Vehicle Safety Enforcement Unit
...
or without cars coupled thereto, operated by a railroad.  (ORC 4511.01(Q))
...
1. {% comment -%} (a) {%- endcomment -%} The right of a vehicle or pedestrian to proceed uninterruptedly in a
...
2. {% comment -%} (b) {%- endcomment -%} A general term denoting land, property or the interest therein,
...
flashing, rotating, or oscillating lights.  (ORC 4511.01(JJJ))
...
route number and so marked.  (ORC 4511.01(JJ))
...
1. {% comment -%} (a) {%- endcomment -%} "Stop or stopping" when prohibited means any halting of a vehicle,
...
2. {% comment -%} (b) {%- endcomment -%} "Stand or standing" means the halting of a vehicle, whether occupied
...
stop signs are erected.  (ORC 4511.01(BBB))

## 301.42   Street or highway; arterial street

<p class="Markdown-list--a-1-A"></p>

1. {% comment -%} (a) {%- endcomment -%} "Street" or "highway" are synonymous and mean the entire width between
the boundary lines of every way open to the use of the public as a thoroughfare
for purposes of vehicular travel.  (ORC 4511.01(BB))
2. {% comment -%} (b) {%- endcomment -%} "Arterial street"  means any United States or State numbered route,
...
as part of a major arterial system of streets or highways.  (ORC 4511.01(CCC))

## 301.43   Through street or highway

   "Through street or highway" means every street or highway as provided in
Section 313.02.  (ORC 4511.01(HH))
...
street or highway or private road open to public travel.  (ORC 4511.01(TT))
...
official traffic control devices.  (ORC 4511.01(PP))
